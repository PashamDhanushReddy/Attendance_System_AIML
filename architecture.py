...
from keras.layers import Conv2D, Activation, Input, Add, MaxPooling2D, Flatten, Dense, Dropout,BatchNormalization, Concatenate, Lambda, add, GlobalAveragePooling2D, ZeroPadding2D, concatenate, AveragePooling2D
...
def InceptionResNetV2():
    ...
    up = Lambda(scaling, output_shape=up.shape[1:], arguments={'scale': 0.17})(up)
    x = add([x, up])
    ...
    up = Lambda(scaling, output_shape=up.shape[1:], arguments={'scale': 0.1})(up)
    x = add([x, up])
    ...
    up = Lambda(scaling, output_shape=up.shape[1:], arguments={'scale': 0.2})(up)
    x = add([x, up])
    ...
    up = Lambda(scaling, output_shape=up.shape[1:], arguments={'scale': 1})(up)
    x = add([x, up])
